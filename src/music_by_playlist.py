...
class Music(object):
    ...
    def saveMusic(self, playlist_id):
        ...
        for item in musicJson.get('playlist').get('trackIds'):
            music_id = item['id']
            try:
                sql.insert_playlist_music(playlist_id, music_id)
            except Exception as e:
                # 打印错误日志
                print(playlist_id, music_id, ' inset db error: ', str(e))
                # traceback.print_exc()
                # time.sleep(1)
...
def saveMusicBatch(user_id, index):
    my_music = Music()
    offset = 9 * index
    playlist = sql.get_playlist_music_page(user_id, offset, 9)
